...
const SESSION_KEY = 'lc_slide_shown';

export default function LearnCatholicSlide() {
  ...
  return (
    <>
      ...
      <div className={`lc-slide-overlay ${isOpen ? 'open' : ''}`} onClick={dismiss} aria-hidden="true" />

      <div className={panelClass} role="dialog" aria-label="Learn Catholic" aria-hidden={!isOpen}>
        {isOpen && <div className="lc-countdown-bar" />}
        <div className="lc-header">
          <button className="lc-close" aria-label="Tutup" onClick={dismiss}>
            <svg viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2"><path d="M18 6L6 18M6 6l12 12" /></svg>
          </button>
          <div className="lc-eyebrow">Learn Catholic</div>
          <h2 className="lc-title">Dalami Iman<br />Katolik Anda</h2>
          <p className="lc-subtitle">Akses ribuan materi ajaran Gereja — KKK, KHK, Kitab Suci, Santo-Santa, dan Vatikan II — gratis, terbuka untuk semua.</p>
        </div>
        <div className="lc-divider" />
        <div className="lc-body">
          <div className="lc-topics-label">Topik Tersedia</div>
          <div className="lc-topics">
            {TOPICS.map((topic) => (
              <div key={topic} className="lc-topic-item">
        ...
        <div className="lc-cta-area">
          ...
          <a href="/public/learn-catholic" className="lc-btn-primary">
            <BookOpenIcon style={{ width: 14, height: 14 }} />
            Mulai Belajar Sekarang
            <ArrowRightIcon style={{ width: 14, height: 14 }} />
          </a>
          <button className="lc-dismiss" onClick={dismiss}>Nanti saja</button>
        </div>
      </div>

      {isDismissed && !isOpen && (
        <button className="lc-reopen-tab" aria-label="Buka Learn Catholic" onClick={() => { setIsDismissed(false); setIsOpen(true); }}>
          <BookOpenIcon style={{ width: 16, height: 16, color: 'var(--color-gold)' }} />
          <span className="lc-tab-text">Learn Catholic</span>
        </button>
      )}
    </>
  );
