...
export default function AdminPendaftaranUmatPage() {
  ...
  const handleLookup = async (e: FormEvent) => {
    ...
    try {
      ...
      if (json.found && json.families?.length > 0) {
        setMatchedFamilies(json.families);
      } else {
  ...
  const handleSelectFamily = (family: any) => {
    ...
    // Auto-fill form from staging data
  ...
  const handleOCRUpload = async (e: React.ChangeEvent<HTMLInputElement>) => {
    ...
    try {
      ...
      // Auto-fill form based on OCR result
    ...
    try {
      const res = await fetch('/api/admin/register-umat', {
        ...
        body: JSON.stringify({
          ...
          lingkungan_slug: formData.lingkungan,
          access_layer: 2,
        }),
      });
    ...
    <div className="min-h-screen p-6" style={{ background: 'linear-gradient(to bottom, var(--color-parchment, #f8f1e2), var(--color-cream, #f5f0e8))' }}>
      <div className="max-w-4xl mx-auto">
        ...
        <div className="flex items-center justify-between mb-6">
          <Link href="/admin/paroki/dashboard" className="inline-flex items-center gap-2 px-4 py-2.5 rounded-lg bg-white/60 hover:bg-white border border-[rgba(200,169,110,0.2)] text-sm font-medium transition-all">
            <ArrowLeftIcon className="h-4 w-4" />
            Kembali ke Dashboard
          </Link>
          <h1 className="text-2xl font-semibold" style={{ fontFamily: FONTS.heading, color: 'var(--color-text-dark, #1a0e05)' }}>
            Pendaftaran Umat Baru
          </h1>
        </div>
              ...
              <div className="space-y-4">
                ...
                {matchedFamilies.length > 0 && (
                  <div className="mt-4 space-y-2">
                    <p className="text-sm font-medium">Data keluarga ditemukan:</p>
            ...
            {activeTab === 'manual' && (
              <div className="space-y-4">
                <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                  ...
                  <div>
                    <label className="block text-sm font-medium mb-1.5">Lingkungan</label>
                    <input
                      type="text"
                      value={formData.lingkungan}
                      onChange={(e) => setFormData(prev => ({ ...prev, lingkungan: e.target.value }))}
                      placeholder="e.g. AR, FA, MRR"
                      className="w-full px-4 py-3 rounded-lg border"
                      style={{ borderColor: 'rgba(200,169,110,0.2)', background: 'var(--color-parchment, #f8f1e2)' }}
                    />
                  </div>
