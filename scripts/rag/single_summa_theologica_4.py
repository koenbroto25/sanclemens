#!/usr/bin/env python3
"""Single doc: Summa_Theologica _4"""
import os,uuid,re,sys,logging,asyncio,json
from pathlib import Path
sys.path.insert(0, os.path.join(os.path.dirname(__file__),'..'))
from rag._shared import get_cockroach_db_connection, get_r2_client, R2_BUCKET, PREVIEW_LEN, embed_text
...
DOC_KEY = 'Summa_Theologica _4'
META = {'nama':'Summa Theologica (Bagian 4)','penulis':'St. Thomas Aquinas','cat':'7a','auth':'reference','kategori':'patristik_skolastik','topic':['patristik','skolastik'],'bot':['bot_3','bot_8','bot_pastor'],'ref':'Summa Theologica (Bagian 4, St. Thomas Aquinas)','domain':'theology'}
def chunk_text(text):
    sections = re.split(r'\n(?=#{1,3} |\n)', text); chunks = []
...
async def main():
    ...
    for idx, chunk in enumerate(chunks):
        ...
        try: emb=await embed_text(chunk); emb=normalize_embedding(emb)
