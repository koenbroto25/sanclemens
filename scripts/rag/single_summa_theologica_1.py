#!/usr/bin/env python3
"""Single doc: Summa_Theologica _1"""
...
# Dynamic DOC_KEY determination
POSSIBLE_DOC_KEYS = ['Summa_Theologica _1', 'Summa_Theologica_1']
DOC_KEY = None
for pk in POSSIBLE_DOC_KEYS:
    if (CLEANED_DIR / f'{pk}.md').exists():
        DOC_KEY = pk
        break
if DOC_KEY is None:
    raise FileNotFoundError(f"Neither 'Summa_Theologica _1.md' nor 'Summa_Theologica_1.md' found in {CLEANED_DIR}")

META = {'nama':'Summa Theologica (Bagian 1)','penulis':'St. Thomas Aquinas','cat':'7a','auth':'reference','kategori':'patristik_skolastik','topic':['patristik','skolastik'],'bot':['bot_3','bot_8','bot_pastor'],'ref':'Summa Theologica (Bagian 1, St. Thomas Aquinas)','domain':'theology'}
def chunk_text(text):
    sections = re.split(r'\n(?=#{1,3} |\n)', text); chunks = []
